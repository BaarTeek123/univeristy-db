...
std::string UI::chooseMainOptions()const{
    system("clear");
    std::cout<<"Student Base program. Choose option which You want to do. "<<std::endl;
...
{
    return std::equal(str1.begin(),str1.end(),str2.begin(),str2.end(),[](char str1, char str2){return tolower(str1) == tolower(str2);});
}
...
void UI::showMenu() {
    while (true){
        ...
        if ((caseIgnoreStringCompare(choice, "q")) || (caseIgnoreStringCompare(choice,  "quit"))) {
            return;
        }
        else if ((caseIgnoreStringCompare(choice, "a")) || (caseIgnoreStringCompare(choice, "add")))
        {
            addStudent();
            std::this_thread::sleep_for(std::chrono::seconds (1));
            std::cout << "You have just succesfully added student to the base." << std::endl;// add
        }
        else if ((caseIgnoreStringCompare(choice, "d")) || (caseIgnoreStringCompare(choice, "display")))
        {
            showDisplayMenu();
        }
        else if ((caseIgnoreStringCompare(choice, "r")) || (caseIgnoreStringCompare(choice,  "reverse")))
        {
            showDeleteMenu();
            //reverse
        }
        else if ((caseIgnoreStringCompare(choice,  "s")) || (caseIgnoreStringCompare(choice,  "search")))
        {
            showSearchMenu();
        }
        else if ((caseIgnoreStringCompare(choice, "p")) || (caseIgnoreStringCompare(choice, "sort")))
        {
            showSortMenu();
        }
...
void UI::showDeleteMenu() {
    ...
    if ((caseIgnoreStringCompare(choice, "n")) ||(caseIgnoreStringCompare(choice, "name"))) {
...
void UI::showSortMenu() {
    ...
    else if ((caseIgnoreStringCompare(choice, "p")) || (caseIgnoreStringCompare(choice, "pesel"))) {
    ...
    else if ((caseIgnoreStringCompare(choice, "i")) || (caseIgnoreStringCompare(choice, "index"))) {
...
Gender setGender(){
    ...
    if ((caseIgnoreStringCompare(option, "m")) || (caseIgnoreStringCompare("male", option)))
        return Gender::Male;
    else if ((caseIgnoreStringCompare( option,  "f")) || (caseIgnoreStringCompare("female", option)))
...
void UI::addStudent() {
    ...
    std::cout<<"\nDoes the student have index number? Input ('y'/'yes' or 'no' / 'n'): ";
    std::getline(std::cin, choice);
    if ((caseIgnoreStringCompare("y", choice)) || (caseIgnoreStringCompare("yes", choice)))
    {
        while (true){
